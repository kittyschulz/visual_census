...
def read_stanford_cars_data(sc_devkit='/Users/katerina/Workspace/visual_census/data/devkit'):
    ...
    value_class_labels = pd.read_csv(os.path.join(sc_devkit, 'value_class.csv'), sep=',',header=None)
    value_class_labels = dict(zip(value_class_labels[0].str.lower(), value_class_labels[1]))
    value_class_names = [value_class_labels[mnfr.lower()] for mnfr in mnfr_label_names]

    type_labels = pd.read_csv(os.path.join(sc_devkit, 'vehicle_type.csv'), header=None)
    type_labels = (type_labels[0].str.lower())
    type_labels = dict(zip(full_label_names,type_labels.str.strip()))
    type_names = [type_labels[full] for full in full_label_names]
    ...
    value_labels = [value_class_names[idx].encode() for idx in class_labels] 
    type_labels = [type_names[idx].encode() for idx in class_labels]

    bboxes = [(y1, x1, y2, x2) for y1, x1, y2, x2 in zip(bb_y1, bb_x1, bb_y2, bb_x2)]

    return full_labels, mnfr_labels, year_labels, value_labels, type_labels, image_fnames, bboxes
...
def image_example(image_string, mnfr_string, year_string, value_string, type_string, full_string, bbox):
    feature = {
        'image_raw': _bytes_feature(image_string),
        'mnfr_label': _bytes_feature(mnfr_string),
        'year_label': _bytes_feature(year_string),
        'value_label': _bytes_feature(value_string),
        'type_label': _bytes_feature(type_string),
...
def write_record(img_path, sc_devkit='/Users/katerina/Workspace/visual_census/data/devkit'):
    full_labels, mnfr_labels, year_labels, value_labels, type_labels, image_fnames, bboxes = read_stanford_cars_data(sc_devkit)

    record_file = 'images.tfrecords'
    with tf.io.TFRecordWriter(record_file) as writer:

        for mnfr_string, year_string, full_string, value_string, type_string, filename, bbox in zip(mnfr_labels, year_labels, full_labels, value_labels, type_labels, image_fnames, bboxes):
            image_string = open(img_path+filename, 'rb').read()
            tf_example = image_example(image_string, mnfr_string, year_string, value_string, type_string, full_string, bbox)
